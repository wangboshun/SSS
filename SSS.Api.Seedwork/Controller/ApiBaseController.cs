    ...
    public abstract class ApiBaseController : ControllerBase
    {
        ...
        public ApiBaseController()
        {
            ...
            _logger = (ILogger)HttpContextService.Current.RequestServices.GetService(
                typeof(ILogger<ApiBaseController>));
            _error = (IErrorHandler)HttpContextService.Current.RequestServices.GetService(typeof(IErrorHandler));

            var userinfo =
                _memorycache.Get<UserInfoOutputDto>(
                    "AuthUserInfo_" + HttpContextService.Current.Request.Headers["Auth"]);
        ...
        protected IActionResult ApiResponse(object data, bool status = true, string message = "", int code = 200)
        {
            ...
            { status = false, data = "", message = Notice.Select(n => n.ErrorMessage), code = 403 });
        }
    }
}